...
resource "aws_eks_cluster" "eks" {
  ...
  vpc_config {
    subnet_ids = concat (
      var.pub_subnet_ids[*].id,
      var.pvt_subnet_ids[*].id
    )
  }
  ...
  depends_on = [
    ...
    module.vpc.resource.aws_subnet.public_subnets,
    module.vpc.resource.aws_subnet.private_subnets
  ]
  tags = {
...
resource "aws_eks_node_group" "nodegroup" {
  ...
  subnet_ids      = module.vpc.aws_subnet.private_subnets[*].id
  scaling_config {
    desired_size = 2
    max_size     = 4
    min_size     = 1
